...
insDecision = """INSERT INTO decision (rg, ville, date_decision, juridiction, description) values (%s,%s,%s,%s,"description")"""
...
for row in cr:
	...
	if (str(row[3])=="x" or str(row[3])==" " or str(row[3])=="" or str(row[3])=="Faux positif DI" or str(row[3])=="error" or str(row[11])=="" or str(row[11])==" " or str(row[11])=="  " or str(row[11])=="irrecevable" ):
		...
		if str(row[2])==rg:
			...
			cur.execute(insDecision, ((str(row[2])), str(row[1]).lower().capitalize(), str(str(year)+"-"+str(month)+"-"+str(day)), (str(row[0]))))
			conn.commit()
		rg=str(row[2])			
